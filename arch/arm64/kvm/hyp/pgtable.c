...
static bool table_matches_block_before_coalescing(
	const struct kvm_pgtable_visit_ctx *ctx,
	struct stage2_map_data *data,
					   ...
					   struct stage2_map_data *data)
{
	...
	/*
	 * A dynamically donated MMIO resource can occupy only part of a host
	 * stage-2 block.  The remaining PTEs may legitimately retain different
	 * attributes, even though none of them contributes a page reference.
	 * Keep the table instead of creating an over-broad block mapping.
	 */
	if (!table_matches_block_before_coalescing(ctx, data, childp, addr))
		return 0;
